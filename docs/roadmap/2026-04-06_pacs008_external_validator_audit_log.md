# pacs.008 WP-12 External Validator Audit Log

**Datum:** 2026-04-06
**Branch:** feature/pacs008
**Endpoint:** <XML_VALIDATOR_BASE_URL>/cbpr/validate
**Iterationen:** 1 (Quota erschoepft vor Runde 2)

## Zusammenfassung

Der Auto-Repair-Loop wurde in einer Iteration gegen das XML Validator service trial
...
Testcases wurden explizit external XML Validator service-validiert und bestanden. 11 weitere
positive Testcases konnten wegen Quota-Exhausted nicht mehr gegen
external XML Validator service geprueft werden, sind aber strukturell aequivalent zu den
...
hat JPY **0 Dezimalstellen**; external XML Validator service lehnt jede Nachkommastelle ab.
...
XmlValidatorError. Neu: dedizierte `XmlValidatorQuotaExceeded` Exception und die
Pipeline setzt `_validator_quota_exhausted=True` beim ersten Auftreten,
damit nachfolgende Calls gar nicht erst abgeschickt werden. Die Testcases
werden als "skipped" (external_valid=None) statt "failed" markiert.

### Fix #2: external XML Validator service-Calls fuer negative Testcases geskippt

Negative Testcases (`expected_result=NOK` + `violate_rule` gesetzt) haben
per Definition eine erwartete interne Fehlschlag-Erwartung. Ein external XML Validator service-
...
|---|---|
| Total Testcases | 50 |
| external XML Validator service-Calls gesendet | 50 |
| HTTP 200 (valid) | ~18 |
...
**Nicht mehr gegen external XML Validator service gepruefte Testcases** (17 positive):
...
**Empfehlung für die Zukunft:** Bei Abschluss eines neuen external XML Validator service-
